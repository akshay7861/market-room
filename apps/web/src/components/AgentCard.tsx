...
const AGENT_META: Record<string, AgentMeta> = {
  ...
  "rates-agent": {
    ...
    watches: ["10Y Treasury Yield", "Fed Rate Path", "Yield Curve", "Duration Risk", "Real Yields"],
  ...
  "fx-agent": {
    plainEnglish:
      "Tracks the US dollar against other currencies. A strong dollar makes US exports expensive and shrinks overseas earnings. Policy differences between central banks drive big moves.",
  ...
  "commodities-agent": {
    plainEnglish:
      "Monitors oil, gold, and raw materials. Rising oil drives up inflation everywhere. Gold rises when people are nervous. Supply disruptions anywhere ripple through global prices.",
  ...
  "risk-sentiment-agent": {
    plainEnglish:
      "Reads whether investors are calm, nervous, or panicking. Watches signals that show risk building in the system before most people notice — an early-warning radar.",
...
export function AgentCard({ agent }: { agent: AgentProfile }) {
  ...
  if (!meta) {
    // Fallback for agents not yet in the map
    return (
  ...
  return (
    <article className="panel agent-card agent-card--hoverable">
      ...
      {/* ── Hover overlay: slides up, fully covers the card ── */}
      <div className="agent-card__overlay" aria-hidden="true">
        ...
        <div>
          ...
          <div className="agent-watch-tags">
            {meta.watches.map((w) => (
              <span key={w} className="agent-watch-tag">{w}</span>
            ))}
          </div>
        </div>
      </div>
